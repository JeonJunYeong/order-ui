...
const {getTwoWayOrderNameList,getTwoWaySymbolInfo,closeSingleOrder,getUserTwoWayOrderRList,openOrder,reverseOpenOrderApi} = twoWayOrderAPi
...
const columns: GridColDef[] = [
    ...
    { field: 'pnl', headerName: 'PNL', width: 130,type:"number" ,
    ...
    {
        field: 'order',headerName: 'Order', width:350,renderCell: (e)=>(
            <div>
                <Button onClick={()=>{saveTwoWayOrder(e)}}>주문</Button>
                <Button onClick={()=>{reverseOpenOrder(e)}}>역계정주문</Button>
                <Button onClick={()=>{singleCloseOrder(e)}}>부분정리</Button>
                <Button onClick={()=>{closePosition(e)}}>주문정리</Button>
            </div>
        )
    }
...
export default function DynamicTab(){
    ...
    const setTabFunc = async () => {
        ...
        setTabList(result.data.map(item => ({ label: item.groupName,id:item.groupId })));
    ...
    const setTabPanel = async (value: number) => {
        const id = tabList[value] ? tabList[value].id : "8d5f0500-819a-4fab-a83d-5074bf972c00"
        const result = await getTwoWayOrderNameList(id);

        setMap(result.data.map(item => {
    ...
    return (
        <Box sx={{height:'600px' }}>
            ...
            {
                tabList.map((e,index)=>{
                    return <CustomTabPanel value={value} index={index} key={index}>
                        <Box sx={{
                            width: '100%',
                            height: 500,
                            borderRadius: 1,
